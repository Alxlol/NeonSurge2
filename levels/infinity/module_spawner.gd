...
func spawn_module():
	...
	if(newest_module): 
		module_instance.position.y = newest_module.get_node('spawn_point').global_position.y - DisplayServer.window_get_size().y
...
func spawn_first_module():
	...
	module_instance.position.y = -DisplayServer.window_get_size().y
	module_instance.scrolling_speed = scrolling_speed
	module_instance.connect('spawn_detection', spawn_module)
	add_child(module_instance)
	newest_module = module_instance
